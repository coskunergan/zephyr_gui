...
status_t LPUART_Init(LPUART_Type *base, const lpuart_config_t *config, uint32_t srcClock_Hz)
{
    ...
    else
    {
        ...
        status = LP_FLEXCOMM_Init(base, LP_FLEXCOMM_PERIPH_LPUART);
...
status_t LPUART_ClearStatusFlags(LPUART_Type *base, uint32_t mask)
{
    ...
    if ((mask & ((uint32_t)kLPUART_TxFifoOverflowInterruptFlag | (uint32_t)kLPUART_RxFifoUnderflowInterruptFlag)) != 0U)
    {
...
status_t LPUART_ReadBlocking(LPUART_Type *base, uint8_t *data, size_t length)
{
    ...
    while (0U != (length--))
    {
...
#endif
        {
            ...
            if (0U != (statusFlag & (uint32_t)kLPUART_RxOverrunInterruptFlag))
            {
                status = ((kStatus_Success == LPUART_ClearStatusFlags(base, (uint32_t)kLPUART_RxOverrunInterruptFlag)) ?
            ...
            if (0U != (statusFlag & (uint32_t)kLPUART_ParityErrorInterruptFlag))
            {
                status =
                    ((kStatus_Success == LPUART_ClearStatusFlags(base, (uint32_t)kLPUART_ParityErrorInterruptFlag)) ?
                         (kStatus_LPUART_ParityError) :
                         (kStatus_LPUART_FlagCannotClearManually));
            }

            if (0U != (statusFlag & (uint32_t)kLPUART_FramingErrorInterruptFlag))
            {
                status =
                    ((kStatus_Success == LPUART_ClearStatusFlags(base, (uint32_t)kLPUART_FramingErrorInterruptFlag)) ?
                         (kStatus_LPUART_FramingError) :
                         (kStatus_LPUART_FlagCannotClearManually));
            }

            if (0U != (statusFlag & (uint32_t)kLPUART_NoiseErrorInterruptFlag))
            {
                status =
                    ((kStatus_Success == LPUART_ClearStatusFlags(base, (uint32_t)kLPUART_NoiseErrorInterruptFlag)) ?
                                 ...
                                 void *userData)
{
    ...
    LP_FLEXCOMM_SetIRQHandler(base, handler.lpflexcomm_handler, handle);
...
void LPUART_TransferHandleIRQ(LPUART_Type *base, void *irqHandle)
{
    ...
    /* If RX overrun. */
    if ((uint32_t)kLPUART_RxOverrunInterruptFlag == ((uint32_t)kLPUART_RxOverrunInterruptFlag & status))
    {
    ...
    if ((0U != ((uint32_t)kLPUART_IdleLineInterruptFlag & status)) &&
    ...
    if ((0U != ((uint32_t)kLPUART_RxDataRegFullInterruptFlag & status)) &&
    ...
    if ((0U != ((uint32_t)kLPUART_TxDataRegEmptyInterruptFlag & status)) &&
...
/* Get the bytes that available at this moment. */
    ...
    if ((0U != ((uint32_t)kLPUART_TransmissionCompleteInterruptFlag & status)) &&
